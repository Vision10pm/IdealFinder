...
DB_PATH = os.path.join(os.path.dirname(MODEL_PATH), 'django-server', 'db.sqlite3')
...
def embedding_to_db(path, target_table, preprocess=False, db=DB_PATH):
    ...
    con = sqlite3.connect(db)
    if preprocess:
        db_df['embedding'] = db_df.iloc[:, 1:129].apply(lambda x: list(x), axis=1)
    db_df = db_df.loc[:, ['image_id_id', 'embedding']]
    print(db_df.image_id_id)
    db_df.to_sql(target_table, con, if_exists='append', index=False)
    con.close()
